...
function intByteLength(value) {
  return Math.max(1, 5 - (((Math.clz32(value) + 3) / 7) | 0));
}
...
function encodeInt64(value, data, start) {
  ...
  if (high === 0) {
    return encodeInt32(low, data, offset);
  }
...
const NATIVE_UTF8_THRESHOLD = 32;
...
function encodeInner(fields, data, start) {
  ...
  for (const f of fields) {
    ...
    switch (field & FIELD_SIZE_MASK) {
      case FIELD_SIZE_32:
        offset = encodeInt32(value, data, offset);
        break;
      case FIELD_SIZE_64:
        offset = encodeInt64(value, data, offset);
        break;
      case FIELD_SIZE_FIXED_32:
        switch (field & FIELD_ENC_MASK) {
          ...
          case FIELD_ENC_IEEE754:
            new DataView(data.buffer, data.byteOffset + start, 4).setFloat32(
              0,
              value
            );
            break;
      ...
      case FIELD_SIZE_UNKNOWN:
        switch (field & FIELD_ENC_MASK) {
          // TODO(indutny): packed?!
          case FIELD_ENC_BYTES:
            offset = encodeInt32(value.length, data, offset);
            data.set(value, offset);
            offset += value.length;
            break;
          case FIELD_ENC_STRING: {
            const len = f.byteLength;
            offset = encodeInt32(len, data, offset);
          ...
          case FIELD_ENC_MESSAGE: {
            offset = encodeInt32(f.byteLength, data, offset);
