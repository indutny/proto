      ...
      case TYPE_VARINT: {
        ...
        switch (fieldSpec) {
          ...
          case FIELD_SINT64: {
            ...
            result.push({ field, value: BigInt(low) | (BigInt(high) << 32n) });
            break;
          }

          case FIELD_BOOL:
            if (b === 0x00) {
              result.push({ field, value: false });
            } else if (b === 0x01) {
              result.push({ field, value: true });
            } else {
          ...
          case FIELD_STRING:
            result.push({
              ...
              value: data instanceof Buffer ?
                data.toString('utf8', start, offset) :
                textDecoder.decode(data.subarray(start, offset)),
            });
            break;

          case FIELD_BYTES:
          case FIELD_HOLE:
            result.push({ field, value: data.subarray(start, offset) });
            break;
        }
