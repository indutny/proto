...
function decodeString(data, start, end) {
  let offset = start;
  // The resulting accumulator is at most the same number of bytes as the
  // original
  const accumulator = new Array(end - start);
  let i = 0;
  while (offset < end) {
    let b1 = data[offset++];
    let b2;
    ...
    let b4;
    let codepoint;
    switch (Math.clz32(~b1 & 0xff)) {
      case 24: // 0x0xxx_xxxx - 1 byte
        codepoint = b1;
        break;
      case 26: // 0b110x_xxxx - 2 bytes
        if (offset === end) {
        ...
        codepoint = ((b1 & 0b0001_1111) << 6) | (b2 & 0b0011_1111);
        break;
      case 27: // 0b1110_xxxx - 3 bytes
        if (offset + 1 === end) {
          codepoint = UTF8_REPLACEMENT;
          break;
        }
        ...
        codepoint = ((b1 & 0b0001_1111) << 12) |
          ((b2 & 0b0011_1111) << 6) |
          (b3 & 0b0011_1111);
        break;
      case 28: // 0b1111_0xxx - 4 bytes
        if (offset + 2 === end) {
          codepoint = UTF8_REPLACEMENT;
          break;
        }
