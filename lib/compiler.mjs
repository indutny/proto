...
export function compile(sources) {
  ...
  typedefs.push(
    'type $DeepNullable<Value> = Value extends Record<string, unknown> ?',
    '  { [key in keyof Value]: $DeepNullable<Value[key]> | undefined } : Value;'
  );
...
function translateMessage(node, syntax, scope, path) {
  ...
  for (const child of node.children) {
    switch (child.kind) {
      ...
      case 'oneof': {
        ...
        for (const [i, field] of child.children.entries()) {
          fieldsWithParents.push({
            parent: child,
            field,
            fieldTypedef,
  ...
  const decoders = [];
  const encoders = [];
  for (const { parent, field, fieldTypedef, isFirst } of fieldsWithParents) {
    ...
    if (type === FIELD_MESSAGE) {
      fieldValue = `${resolvedField.join('.')}.decode(data, value.start, value.end)`;
    } else {
      fieldValue = 'value';
    }
    ...
    // Encoder
    // TODO(indutny): compare against default value too?
    // TODO(indutny): convert integers to unsigned
    if (isOneof) {
      encoders.push(
        `    ${isFirst ? 'if' : 'else if'} ` +
          `(data.${parent.name}?.kind === '${field.name}') {`,
        '      fields.push({',
        `        id: ${field.id}, field: ${type},`,
        `        value: data.${parent.name}.value`,
        '      });',
        '    }'
      );
    } else if (isRepeated) {
      ...
      encoders.push(
        ...
        `        fields.push({ id: ${field.id}, field: ${type}, value });`,
        `      }`,
        '    }'
      );
    } else {
      encoders.push(
        `    if (data.${field.name} !== undefined) {`,
        '      fields.push({',
        `        id: ${field.id}, field: ${type}, value: data.${field.name}`,
        '      });',
        '    }'
      );
    }
  ...
  // Generated function: decode(data)
  ...
  // Generated function: toFields(data)
  source.push('  toFields(data) {');
  source.push('    const fields = [];');
  for (const e of encoders) {
    source.push(e);
  }
  source.push('    return fields;');
  source.push('  },');

  // Generated function: encode(data)
  typedefs.push(
    `  export function encode(data: $DeepNullable<${node.name}>): Uint8Array;`
  );

  source.push(
    '  encode(data) {',
    '    return $encode(this.toFields(data));',
    '  }'
  );
