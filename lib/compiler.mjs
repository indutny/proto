...
function getFieldTypedef(field, type, name) {
  switch (type) {
    case FIELD_INT32:
    case FIELD_UINT32:
    case FIELD_SINT32:
...
function translateMessage(node, syntax, scope, path) {
  ...
  for (const { parent, field, fieldTypedef } of fieldsWithParents) {
    ...
    if (isOneof) {
      fieldTypedef.push(
        `{ kind: '${field.name}', ` +
          `value: ${getFieldTypedef(field, type, resolvedField).join(' | ')} }`
      );
      // Template populated once per parent above
    } else if (isRepeated) {
      fieldTypedef.push(
        `Array<${getFieldTypedef(field, type, resolvedField).join(' | ')}>`
      );
      template.push(`${field.name}: []`);
    } else {
      defaultValue = options.get('default');
      fieldTypedef.push(...getFieldTypedef(field, type, resolvedField));
      switch (type) {
        case FIELD_INT32:
        case FIELD_UINT32:
        case FIELD_SINT32:
