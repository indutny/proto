...
export function compile(sources) {
    ...
    for (const node of root) {
      switch (node.kind) {
        ...
        case 'enum': {
          ...
          target.set(node.name, result.source);
          break;
        }
        default:
          // Ignore
  ...
  for (const [name, value] of internal) {
    out.push(`const ${name} = ${value};`);
  }

  for (const [name, value] of exports) {
    out.push(`export const ${name} = ${flattenExports(value)};`);
  }
...
function translateMessage(node, syntax, scope, path) {
  ...
  typedefs.push('  $unknown: Array<Uint8Array>;');
  // TODO(indutny): populate me
  ...
  typedefs.push(`export namespace ${node.name} {`);
  typedefs.push('  export function decode(');
  typedefs.push('    data: Uint8Array, start?: number, end?: number,');
  typedefs.push(`  ): ${node.name};`);
  typedefs.push('}');
