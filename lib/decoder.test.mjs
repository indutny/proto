...
describe('64-bit integers', () => {
  ...
  test('unknown field', () => {
    const data = [(1 << 3) | TYPE_VARINT, 0x80, 0x81, 0x82, 0x83, 0x84, 0x05];
    expect(harness(data, [FIELD_UNKNOWN])).toEqual([
      { id: 1, value: new Uint8Array(data) },
    ]);
  });
...
describe('len', () => {
  ...
  test('message', () => {
    const data = [(1 << 3) | TYPE_LEN, 0x03, 0x01, 0x02, 0x03];
    expect(harness(data, [FIELD_MESSAGE])).toEqual([
      { id: 1, value: { start: 2, end: 5 } },
    ]);
  });
...
describe('32-bit fixed', () => {
  ...
  test('unknown field', () => {
    const data = [(1 << 3) | TYPE_I32, 0x01, 0x02, 0x03, 0x04];
    expect(harness(data, [FIELD_UNKNOWN])).toEqual([
      { id: 1, value: new Uint8Array(data) },
    ]);
  });
