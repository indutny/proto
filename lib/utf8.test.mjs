...
import { decodeString, stringByteLength, encodeStringInto } from './utf8.mjs';
...
describe('stringByteLength', () => {
  ...
  test('ascii string', () => {
    expect(stringByteLength('hello\x7f')).toBe(6);
  });
  ...
  test('three-byte codes', () => {
    expect(stringByteLength('\u0800\ud7ff\ue000\uffff')).toBe(12);
  });

  test('four-byte codes', () => {
    expect(stringByteLength('\ud800\udc00\udbff\udfff')).toBe(8);
  });

  test('missing low surrogate', () => {
    expect(stringByteLength('\ud800a')).toBe(4);
  });
...
describe('encodeStringInto', () => {
  function roundtrip(value) {
    const encoder = new TextEncoder();
    const actual = new Uint8Array(128);
    const expected = encoder.encode(value);
    expect(encodeStringInto(value, actual, 0)).toBe(expected.length);
